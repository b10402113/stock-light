...
- 架構與模組開發規範: 見 `docs/architecture.md` (含跨模組限制、目錄結構、Anti-patterns)
- 數據庫設計與遷移: 見 `docs/database.md` (含索引策略、大表處理、Alembic 規範)
- API 與資料驗證規範: 見 `docs/api-spec.md` (含 Pydantic 驗證、游標分頁實作、JWT 認證)
- 背景任務與非同步決策: 見 `docs/async-tasks.md` (含 Celery vs BackgroundTasks 決策表)
- 測試規範: 見 `docs/testing.md` (含 AsyncClient 測試與 Dependencies 覆寫)
